'use client';

import { useEffect, useMemo, useRef, useState } from 'react';
import type { User } from 'firebase/auth';

import { ANALYTICS_EVENTS } from '../lib/analytics-events';
import { captureEvent } from '../lib/posthog';
import { ActionItemSubjectLink } from './ActionItemSubjectLink';
import type { ActionItemDoc, ActionItemLane } from '../lib/action-item-types';

/**
 * Anniversary + referral action item card.
 *
 * SOURCE OF TRUTH: `CONTEXT.md` > `Channel Rules` > Agent action item
 * surface. Both lanes use the same vocabulary
 * (`['text_personally', 'call', 'skip']`) and don't carry a pre-filled
 * SMS body — the agent decides what to say. So both render the same
 * minimal card with three CTAs: Call (tel:), Text (sms: address only),
 * and Skip.
 *
 * Distinction from the welcome / retention text cards:
 * - No pre-filled SMS body → no Copy or QR (nothing to copy/encode)
 * - No body preview
 *
 * Distinction from the retention call card:
 * - Adds a Text CTA (anniversary/referral surfaces both options;
 *   retention's call card is single-purpose because retention's text
 *   stage is its own card 48h later)
 */

interface AnniversaryReferralActionItemCardProps {
  item: ActionItemDoc;
  user: User | null;
  onCompleted?: () => void;
}

function ageBucket(days: number, lane: ActionItemLane): 'fresh' | 'aging' | 'urgent' {
  // Anniversary expires at 30d, referral at 14d. Color shift just
  // mirrors the same proportional curve as the welcome card so the
  // visual language stays consistent.
  const expirationDays = lane === 'anniversary' ? 30 : 14;
  const ratio = days / expirationDays;
  if (ratio < 0.3) return 'fresh';
  if (ratio < 0.7) return 'aging';
  return 'urgent';
}

function classesForAge(age: ReturnType<typeof ageBucket>): { container: string; pill: string } {
  if (age === 'urgent') {
    return {
      container: 'border-[#f3a8a8] bg-[#fff5f5]',
      pill: 'bg-[#fde6e6] text-[#b42318]',
    };
  }
  if (age === 'aging') {
    return {
      container: 'border-[#f1c97a] bg-[#fff8e8]',
      pill: 'bg-[#fde9c4] text-[#7a4a00]',
    };
  }
  return {
    container: 'border-[#d0d0d0] bg-white',
    pill: 'bg-[#f0f0f0] text-[#5f5f5f]',
  };
}

function ageLabel(days: number): string {
  if (days <= 0) return 'Today';
  if (days === 1) return '1d ago';
  return `${days}d ago`;
}

function laneTitle(lane: ActionItemLane, firstName: string): string {
  if (lane === 'anniversary') return `Reach out to ${firstName} (anniversary)`;
  if (lane === 'referral') return `Follow up with ${firstName} (referral)`;
  return `Reach out to ${firstName}`;
}

function laneSubtitle(lane: ActionItemLane, triggerReason: string): string {
  if (lane === 'anniversary') {
    if (triggerReason === 'anniversary_push_revoked') return 'push revoked — automation paused';
    if (triggerReason === 'anniversary_push_send_failed') return 'push delivery failed';
    return 'no push channel — automation paused';
  }
  if (lane === 'referral') {
    return '24h after last drip — no reply';
  }
  return triggerReason;
}

export default function AnniversaryReferralActionItemCard({
  item,
  user,
  onCompleted,
}: AnniversaryReferralActionItemCardProps) {
  const phone = item.displayContext.subjectPhoneE164 || '';
  const subjectFirst =
    item.displayContext.subjectFirstName ||
    item.displayContext.subjectName ||
    'Client';

  const ageDays = useMemo(() => {
    const created = Date.parse(item.createdAt);
    if (!Number.isFinite(created)) return 0;
    return Math.floor((Date.now() - created) / (1000 * 60 * 60 * 24));
  }, [item.createdAt]);

  const age = ageBucket(ageDays, item.lane);
  const styles = classesForAge(age);

  const [completing, setCompleting] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const viewedRef = useRef(false);

  useEffect(() => {
    if (viewedRef.current) return;
    viewedRef.current = true;
    if (!user) return;
    captureEvent(ANALYTICS_EVENTS.ACTION_ITEM_VIEWED, {
      lane: item.lane,
      trigger_reason: item.triggerReason,
      view_count_after: item.viewCount + 1,
      age_days: ageDays,
    });
    (async () => {
      try {
        const token = await user.getIdToken();
        await fetch(`/api/agent/action-items/${item.itemId}/view`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
        });
      } catch (err) {
        console.error('[anniversary-referral-card] mark-viewed failed (non-blocking)', err);
      }
    })();
  }, [user, item.itemId, item.triggerReason, item.viewCount, ageDays, item.lane]);

  const completeWith = async (
    completionAction: 'call' | 'text_personally' | 'skip',
    completionNote?: string | null,
  ) => {
    if (!user || completing) return;
    setCompleting(true);
    setErrorMsg(null);
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/agent/action-items/${item.itemId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ completionAction, completionNote: completionNote ?? null }),
      });
      if (!res.ok) {
        const j = (await res.json().catch(() => null)) as { error?: string } | null;
        throw new Error(j?.error || `complete failed (${res.status})`);
      }
      captureEvent(ANALYTICS_EVENTS.ACTION_ITEM_COMPLETED, {
        lane: item.lane,
        trigger_reason: item.triggerReason,
        completion_action: completionAction,
        age_days: ageDays,
        view_count_at_completion: item.viewCount + 1,
      });
      onCompleted?.();
    } catch (err) {
      setErrorMsg(err instanceof Error ? err.message : 'Could not record action.');
    } finally {
      setCompleting(false);
    }
  };

  const telHref = phone ? `tel:${phone}` : null;
  const smsHref = phone ? `sms:${phone}` : null;

  return (
    <article className={`rounded-xl border-2 px-4 py-3 transition-colors ${styles.container}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-bold text-[#0D4D4D]">
            <ActionItemSubjectLink clientId={item.clientId} prospectId={item.prospectId}>{laneTitle(item.lane, subjectFirst)}</ActionItemSubjectLink>
          </p>
          <p className="text-[12px] text-[#4f4f4f] mt-0.5">
            {phone || 'No phone on file'} · {laneSubtitle(item.lane, item.triggerReason)}
          </p>
        </div>
        <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${styles.pill}`}>
          {ageLabel(ageDays)}
        </span>
      </div>

      {errorMsg ? (
        <p className="mt-2 text-[11px] font-semibold text-[#b42318]">{errorMsg}</p>
      ) : null}

      <div className="mt-3 flex flex-wrap items-center justify-end gap-2">
        <button
          type="button"
          onClick={() => { void completeWith('skip'); }}
          disabled={completing}
          className="inline-flex items-center gap-1 rounded-lg border border-[#d0d0d0] bg-white px-3 py-1.5 text-xs font-bold text-[#5f5f5f] hover:bg-gray-50 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          Skip
        </button>
        {smsHref ? (
          <a
            href={smsHref}
            onClick={() => { void completeWith('text_personally'); }}
            className="inline-flex items-center gap-1 rounded-lg border border-[#0D4D4D] bg-white px-3 py-1.5 text-xs font-bold text-[#0D4D4D] hover:bg-gray-50 active:scale-[0.97] transition"
          >
            💬 Text
          </a>
        ) : null}
        {telHref ? (
          <a
            href={telHref}
            onClick={() => { void completeWith('call'); }}
            className="inline-flex items-center gap-1 rounded-lg bg-[#3DD6C3] px-3 py-1.5 text-xs font-bold text-[#0D4D4D] hover:bg-[#32c4b2] active:scale-[0.97] transition"
          >
            📞 Call
          </a>
        ) : null}
      </div>
    </article>
  );
}
