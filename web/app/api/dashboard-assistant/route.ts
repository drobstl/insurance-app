...
3. **[Leads](/dashboard/leads)** — The pre-sale pipeline. Where prospective customers live before they close. Agents can:
   ...
   - **Auto push reminders** (Chunk 4f-extension): if the lead downloaded the AFL app and granted notifications, a cron auto-pushes them a reminder N hours before the appointment (per-agent timing in [Settings → Appointments & Leads](/dashboard/settings)).
...
5. **[Referrals](/dashboard/referrals)** — The full referral pipeline. When a client refers someone:
   ...
   - To enable: go to [Settings → Messages](/dashboard/settings) and toggle the AI assistant on. You must also add a scheduling link (Calendly, Cal.com, etc.).
...
7. **[Rewrites](/dashboard/policy-reviews)** — Policy anniversary and rewrite alerts. When a policy approaches its 1-year anniversary:
   ...
   - Two message styles available: "check in" (relationship-first) or "lower price" (savings-first). Set your preference in [Settings → Messages](/dashboard/settings).
...
10. **[Settings](/dashboard/settings)** — Five tabs:
    - **Profile**: Name, phone, email, headshot photo, scheduling link, and NPN. The photo + name appear in the client's mobile app + on the vCard contact card. Profile also holds:
    ...
    - **Messages**: All the agent→lead/client text in one place — toggle the AFL referral assistant on/off, the lead intro text, the dial script (shown during a live call), the referral introduction message, the client welcome text, anniversary message style, auto-holiday cards, and rewrite (policy-review) campaigns.
    - **Appointments & Leads**: phone-vs-video default, default meeting link (Zoom personal room or Meet permalink), auto-create Google Meet toggle (requires the Calendar connection in Account), the **auto push-reminder timing** (hours before an appointment; 0 = disabled), dial persistence, and **Lead-home videos**: per-agent intro + FAQ + case-study uploads that play in the lead's AFL mobile app on the /lead-home screen.
    - **Account**: View your subscription tier, price, and trial status. "Manage" opens the Stripe billing portal where you can change plan, update card, or cancel. Also: **Google Drive** + **Google Calendar** OAuth (Calendar drives one-way appointment sync, the day-strip conflict view, and the optional auto-generated Google Meet link per video appointment), invite agents (recruit), change email, and change password.
...
- **"How do I upload videos for my leads?"** → [Settings → Appointments & Leads → Lead-home videos](/dashboard/settings). Upload an intro video (plays at the top of the lead's AFL app), plus any FAQ videos or case-study videos. Each video can be up to 200 MB (.mp4, .mov, or .webm). Without uploads the lead-home looks visually empty.
