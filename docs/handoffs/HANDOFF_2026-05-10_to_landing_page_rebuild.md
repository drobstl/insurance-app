...
| ~June 9 | Delete `/api/beneficiary/send-intro` server route | 30-day cooldown post-beneficiary-invite cutover (May 10). Marked `@deprecated` already with console warning. Web "Send Beneficiary Intro" buttons + composer drawer in `ClientDetailModal.tsx` already removed May 10 (Linq-line sender was wrong channel for v3.1 invite-only). |
| As cleanup | Delete `/api/beneficiary/send-manual` server route | No remaining callers as of May 10 — sole caller was the "Jump in manually" textbox in `ClientDetailModal.tsx`, removed alongside the intro cleanup. Route still works but is orphaned; safe to delete in a focused commit. |
| Revisit | **Agent-side beneficiary affordances in dashboard** | Two distinct things were removed from the client modal May 10, both worth revisiting if real demand surfaces: (1) **Agent-initiated invite** — mobile Invite button is canonical (opens agent's Messages app with prefilled SMS). Open question: do agents want the same affordance in the desktop dashboard for cases where the policyholder doesn't initiate? If yes, must be personal-phone hand-off (sms:/clipboard), NOT Linq, and copy differs from the policyholder→beneficiary version. (2) **Take-the-wheel manual message into an active beneficiary thread** — the use case is real (agent interjecting personally mid-AI-conversation), but the client detail modal was a weird discoverability path for it; agents don't watch client modals to follow live conversations. Belongs in whatever surface shows live beneficiary conversations — Action Items handoff, a future inbox view, etc. — with fresh UX, not buried inline. |
