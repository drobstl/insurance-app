...
>
> **Last re-ranked:** June 2, 2026 evening — the entry-mechanism cutover shipped end-to-end (no-card 14-day trial #89, post-trial Free back wall + enforcement #94/#96/#97), so the front-door work moved to Recently shipped and the open priorities are now the `/pricing` cutover, the remaining Free-tier caps, the Performance MVP, and the commission engine. Prior (May 31): folded in the May 30 Growth + Distribution Lock (no-card 14-day Pro entry, permanent Free tier, Performance gating, Starter migration, FirstPromoter affiliate, IMO leader free seat, FFL/Rob distribution priority).
>
...
| 🟢 **Free tier feature gating + caps** | 1–2 days | **Core enforcement SHIPPED (#94 / #96 / #97):** Free is now *data-preserved + engine-paused* — every client-facing outreach cron skips Free agents (`isFreeTier` in `web/lib/tier-gating.ts`), parsing **new** application PDFs is blocked at `parseApplicationFile` (locked extraction pipeline untouched), and import-batch immediate drips are suppressed. **Note:** this supersedes the earlier "cadences ENABLED on those 25" idea — Free agents get NO client-facing automation. **Remaining:** explicit 25-contact write cap on `clients`, swap the hard new-PDF block for a 5-PDF/month meter, and hide Activity Policy Ledger / bulk-import / pre-sale surfaces with tier-naming upgrade prompts. |
