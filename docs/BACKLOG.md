...
| Item | Effort | Notes |
|---|---|---|
| ✅ **Performance / Coaching page** | SHIPPED #137 (Jun 8) | Standalone `/dashboard/coaching`: paste a transcript → **R.E.A.L.** scores (Rapport / Emotion / Assumption / Lock-It-Down) + checkpoints against the agent's **own** script + verdict + Top-3 priorities. Engine **ported from Closr** (Closr sunset; AFL owns it). Metered: Free locked, Growth 4/mo, Pro+ unlimited. |
| 🟡 **Coaching: "Score this call" from a lead (contextual entry)** | 1–2 days *(blocked on transcripts)* | A "Score this call" button on a lead's call history, so coaching is one tap from where calls happen instead of pasting into the standalone page. **Blocked until calls are recorded/transcribed and tied to a lead** — today the dialer only logs an outcome, no transcript. Pair with call recording/transcription when that lands. |
...
| 🟡 Team Performance dashboard (leaderboards + coaching priorities widget) | 2–3 days | Sits on top of individual Performance (now SHIPPED #137, on the **R.E.A.L.** framework). Rob's call action item. **⚠️ Check PR #140 (admin-performance-rollup) FIRST — it may already be this work; if so, make sure it scores/aggregates on the SAME R.E.A.L. framework the individual Coaching page uses, so the two don't diverge.** |
