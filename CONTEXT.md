...
- Shipped (May 12–18, 2026): **Pre-application lead management ("lead mode") — Phase 1 surface built end-to-end on `claude/charming-satoshi-3be4cc`.** ~30 commits across one week building the entire pre-application surface. Branch is local-only; Daniel has not yet deployed to prod. The deferred feature-flag work that lets him deploy with lead mode invisible is the open item; see `docs/handoffs/HANDOFF_2026-05-18_evening_to_queue_polish.md`. Highlights, in roughly chronological order:
  ...
  - **Open at end of May 18 session** (handed off to next session at `docs/handoffs/HANDOFF_2026-05-18_evening_to_queue_polish.md`):
    ...
    - Feature flag for the entire pre-application lead-mode surface (Daniel's prep work to deploy AFL to prod with lead mode invisible to live agents). Deferred until Daniel is ready to push lead mode to prod.
...
- **Lead-mode branch (`claude/charming-satoshi-3be4cc`) is not pushed.** Local-only at HEAD `8ee2a0c` as of May 18, 2026. Carries the entire pre-application lead-mode surface + the May 18 mobile notification deferral + the call-queue two-pane refactor. Feature-flag prep work is the gating item before Daniel can deploy AFL to prod with lead mode invisible. See `docs/handoffs/HANDOFF_2026-05-18_evening_to_queue_polish.md`.
- **Call queue right-pane: list rail is not sticky.** When the right pane is long (full lead profile + appointments + dial history) the narrow list rail scrolls out from under the agent's finger. Next-session fix; design and trade-offs in the handoff doc.
